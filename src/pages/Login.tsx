...
export default function Login() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      console.log("Respon dari Laravel:", data);
  ...
  const handleAppleLogin = async () => {
    ...
    try {
      ...
      const user = result.user;
      console.log("Login Apple Berhasil:", user);

      navigate('/', { replace: true });
    } catch (err) {
      console.error(err);
      setError('Gagal login dengan Apple. Pastikan domain terverifikasi.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    flex flex-col font-sans text-white pt-24">

<Header/>
      <div className="flex-1 flex items-center justify-center pt-4 md:pt-3 p-6">
        
        <div className="flex w-full max-w-5xl bg-white text-gray-800 rounded-xl overflow-hidden shadow-2xl min-h-[480px] ">
          
          <div className="hidden md:block w-1/2 relative">
            <img 
              src="/images/LOGIN pesawat.jpg" 
              alt="Airplane over ocean" 
              className="absolute inset-0 w-full h-full object-cover"
            />
            <div className="absolute inset-0 bg-black/10"></div>
          </div>
          <div className="w-full md:w-1/2 bg-[#FFF8E7] p-8 md:p-12 flex flex-col justify-center">
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-lg border border-gray-300 bg-transparent focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#FFC086] focus:border-transparent transition-all"
                    required
                  />
                </div>
                ...
                <div className="flex items-center justify-between text-sm mt-2">
                  <label className="flex items-center gap-2 cursor-pointer select-none text-gray-600">
                    <input 
                      ...
                      className="rounded border-gray-300 text-[#FFC086] focus:ring-[#FFC086]"
                    />
                    Remember me
                  </label>
                  <Link to="/forgot-password" className="font-semibold text-gray-700 hover:text-black transition">
                    Forgot password?
                  </Link>
                </div>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-2.5 px-4 bg-[#FFC086] hover:bg-[#ffb066] text-white font-bold rounded-lg shadow-md hover:shadow-lg transition-all mt-3 transform active:scale-95"
                >
                ...
                <div className="space-y-2 pt-2">
                  ...
                  <button 
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-[#E6E6E6] hover:bg-[#d4d4d4] py-3 rounded-lg text-sm font-medium text-gray-700 transition-colors"
                  >
